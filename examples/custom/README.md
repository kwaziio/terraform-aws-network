...
This example is intended to show a custom use case for this module, where features are disabled and created manually by the adopter.
...
* [https://github.com/kwaziio/terraform-aws-network/tree/main/examples/custom](https://github.com/kwaziio/terraform-aws-network/tree/main/examples/custom)

### Deploying Custom Example as Integration Test

The following commands will initialize and deploy the infrastructure for the custom example:

```SHELL
terraform -chdir=examples/custom init -reconfigure
terraform -chdir=examples/custom apply
```

### Destroying Custom Example After Integration Test

The following command will destroy any resources created while deploying the custom example:

```SHELL
terraform -chdir=examples/custom destroy
```
